...
All rates are temperature-dependent and defined on a per-day basis, then converted to tick-level probabilities via a Poisson process:
\begin{equation}
    p = 1 - \exp(-r \Delta t),
\end{equation}
where $\Delta t$ is the tick duration in days ($\Delta t = 0.25 \text{ h} = 0.01042 \text{ days}$).

The \textbf{daily projection matrix} is defined as:
\begin{equation}
\mathbf{M}(T) = 
\begin{pmatrix}
(1 - p_E^{\text{dev}}) p_E^{\text{surv}} & 0 & 0 & F \cdot K \\
p_E^{\text{dev}} p_E^{\text{surv}} & (1 - p_L^{\text{dev}}) p_L^{\text{surv}} & 0 & 0 \\
0 & p_L^{\text{dev}} p_L^{\text{surv}} & (1 - p_P^{\text{dev}}) p_P^{\text{surv}} & 0 \\
0 & 0 & p_P^{\text{dev}} p_P^{\text{surv}} & p_A^{\text{surv}}
\end{pmatrix},
\end{equation}
with stage abundances evolving as $[E,L,P,A]_{t+1}^\top = \mathbf{M}(T) [E,L,P,A]_t^\top$.

The temperature-dependent functions and ecological constraints are summarized in Table \ref{tab:lifecycle_equations}.

\begin{table}[!htb]
\centering
\caption{Mathematical formulations for temperature-dependent lifecycle processes.}
\label{tab:lifecycle_equations}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Component} & \textbf{Mathematical Definition} \\ \midrule
Development Rate ($d_X$) & $d_X(T) = \max(0, a_X T^2 + b_X T + c_X) \quad \text{for } X \in \{E,L,P\}$ \\
Dev. Probability ($p_X^{\text{dev}}$) & $p_X^{\text{dev}} = 1 - \exp(-d_X(T) \cdot 1 \text{ day})$ \\
Stage Survival ($S_X$) & $S_X(T) = \text{amp}_X \cdot \exp\left(-\frac{1}{2}\left(\frac{T - \mu_X}{\sigma_X}\right)^2\right)$ \\
Daily Mortality ($\mu_X$) & $\mu_X(T) = -\ln\bigl(S_X(T)\bigr) \cdot d_X(T) \quad (\text{if } S_X \le 0, \mu_X = 1)$ \\
Daily Survival ($p_X^{\text{surv}}$) & $p_X^{\text{surv}} = \exp\bigl(-\mu_X(T) \cdot 1 \text{ day}\bigr) = S_X(T)^{d_X(T)}$ \\
Adult Survival ($p_A^{\text{surv}}$) & $\mu_A(T) = \max(0, a_A T^2 + b_A T + c_A); \quad p_A^{\text{surv}} = \exp(-\mu_A(T))$ \\
Fecundity ($F$) & $F(T) = \max\left(0, a_F e^{b_F T} - e^{b_F T_{\max} - \left(\frac{T_{\max}-T}{c_F}\right)^2}\right)$ \\
Host Scaling ($K$) & $K = 1 + \ln(1 + \text{population density})$ \\ 
\bottomrule
\end{tabular}
\end{table}

\textbf{Density-Dependent Mortality:} Each aquatic habitat has a carrying capacity $C$ scaled by water volume. If larvae $L > C$, 
excess individuals are removed via a probabilistic survival filter.

All temperature-response coefficients are stored in a YAML configuration file under the \texttt{species} key, making the model 
generic and easily adaptable (see Table \ref{tab:lifecycle_params}).

The key methodological refinements include:
\begin{enumerate}
    \item \textbf{High-Frequency Resolution:} Mortality is applied every tick as a daily probability rather than only upon stage completion.
    \item \textbf{Derived Mortality:} $\mu_X(T)$ is derived from the Gaussian survival function and development rate, ensuring consistency 
    with the underlying ODE system.
    \item \textbf{Matrix Decoupling:} Matrix entries explicitly separate development and survival following discrete-time approximations.
\end{enumerate}

\begin{table}[!htb]
\centering
\caption{Generic temperature-response parameters and YAML configuration mapping.}
\label{tab:lifecycle_params}
\begin{tabular}{llll}
\toprule
\textbf{Parameter Set} & \textbf{Meaning} & \textbf{Unit} & \textbf{YAML Key} \\ \midrule
$d_E, d_L, d_P$ & Dev. rates (coeffs) & $1/^\circ\text{C}^n$ & \texttt{[stage]\_dev\_a,b,c} \\
$S_E, S_L, S_P$ & Survival (amp, $\mu, \sigma$) & $-, ^\circ\text{C}, ^\circ\text{C}$ & \texttt{[stage]\_survival\_amp,mean,sigma} \\
$\mu_A$ & Adult mortality (coeffs) & $1/^\circ\text{C}^n$ & \texttt{adult\_mort\_a,b,c} \\
$F$ & Fecundity (coeffs) & Varies & \texttt{fecundity\_a,b,Tmax,c} \\
$K$ & Host factor & dim. & (derived) \\ \bottomrule
\end{tabular}
\end{table}
